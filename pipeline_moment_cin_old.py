...
#Pour la sauvegarde
save_output = False  #Mettre False pour ne rien sauvegarder
save_final = True

dir_save = '/home/users/mnt/averliat/analyses/B335_noturb_norot_hydro_hr2/'

dir_save_output = 'save_output'
file_save_output = '/save_output_'

file_save = dir_save + 'Moment_cinetique_tot_output_'+str(num_min)+'_'+str(num_max)+'_decentre.fits'
file_save_2 = dir_save + 'Integrale_valeurs_absolues_moments_cinetiques_output_'+str(num_min)+'_'+str(num_max)+'_decentre.fits'
...
for l in range(num_tot):
	...
	if os.path.isfile(dir_save + dir_save_output + file_save_output + str(num) + '.npz') == False:
		...
		#--------------------------------------------
		if save_output == True:
			np.savez(dir_save + dir_save_output + file_save_output + str(num), dx=dx, pos=pos, centre=centre, vel=vel, rho=rho)

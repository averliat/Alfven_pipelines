...
import trace_shell_moment_cin_observateurs2 as tsmco2
...
for i in range(len(tag)):
    i=len(tag)-i-1
    tsmco2.trace_moment_cin_disque(base_simu+tag[i],tag[i],'None',1,output_max[i],width_bar[i],hist,diff_abs,diff_relat,legend[i],marker[i],t1[i],moyenne_glissante=moyenne_glissante)

...
#-------------------------------------------------------
simu = 'jet_fullM_rotNS_dirJ/temp'#'B335_noturb_norot_hydro_pert_asym_aleatoire_lllr_bigbox_10pourc_sink'
...
#---------------------------------------------
m_sinks_tab=np.zeros(output_max+1-output_min)
m_gas_tab=np.zeros(output_max+1-output_min)
m_tot_tab=np.zeros(output_max+1-output_min)
...
for num_output in range(output_min,output_max+1):
    ...
    try:
        ...
        m_sinks[ind_output]=None
        m_gas[ind_output]=None
        M_tot[ind_output]=None
    ...
    m_sinks_tab[ind_output]=m_sinks
    m_gas_tab[ind_output]=mass_tot
    m_tot_tab[ind_output]=mass_tot+m_sinks
    numero_output[ind_output]=num_output

    if num_output==output_min:
        M_0 = mass_tot
...
plt.figure()
#plt.plot(numero_output,m_sinks_tab, marker='.', color='b')
plt.plot(numero_output,M_0-m_sinks_tab, marker='.', color='b', label=r'$M_0-M_{sinks}$')
plt.plot(numero_output,m_gas_tab, marker='.', color='r', label=r'$M_{gas}$')
plt.plot(numero_output,m_tot_tab, marker='.', color='g', label=r'$M_{tot}$')
plt.legend(loc='best')

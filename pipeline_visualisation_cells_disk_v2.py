...
#-------------------------------------------------------
simu = 'B335_noturb_norot_hydro_pert_asym_aleatoire_bigbox_50pourc_sink_seuil_haut_niMHD_lr'
...
def disk_op(dset):
    ...
    mask2 = vrad_orth_ratio < 0.5 #test
    mask[mask]=mask2
    flag[mask] = 1.0
    flag[~mask] = 0.0 
    return flag
...
def carte_dens_vel(view_diagram,up_vector,pos_colorbar_dens,pos_colorbar_vel,tag_compl):
    ...
    cbar.set_label(r'$\text{log} \left( N \right) \, \, \left( cm^{-2} \right)$')
    if radius_zoom==5:
        plt.xlim([0,lbox_au])
        plt.ylim([0,lbox_au])
    if save==True:
...
for view in range(3):
    ...
    tag_compl=view_diagram[view]+'_'
    carte_dens_vel(view_diagram[view],up_vector[view],pos_colorbar_dens,pos_colorbar_vel,tag_compl)
#=================================================
#=================================================

...
#from hop_camille.hop import HOP              #ANTOINE VERLIAT : commente car sinon extrac_disk plante car module absent
...
#self.amr_field_descrs_by_file["3D"] = {"hydro" : [ output.Scalar("rho", 0), output.Vector("vel", [1, 2, 3]),
#            output.Vector("Bl", [4,5,6]), output.Vector("Br", [7,8,9]),
#            output.Scalar("P", 10), output.Scalar("ext", 11)] }
#           "grav"  : [ output.Scalar("phi", 0), output.Vector("g", [1, 2, 3]) ] }

#print RamsesOutput.amr_field_descrs_by_file
...
########################################################################################
########################################################################################                 
def get_pdf(path_in,num,path_out=None,force=False,ps=False,xmin=0,xmax=1.,ymin=0,ymax=1.,zmin=0.,zmax=1.,tag='',mag_im=True,order='<'):
...
#        pc,Ms,Myr,scale_n,scale_d,scale_t,scale_l,scale_v,scale_T2,scale_ener,scale_mag,microG,km_s,Cwnm,scale_mass,unit_col,lbox_pc = normalisation(lbox)
        ...
        pc,Ms,Myr,scale_n,scale_d,scale_t,scale_l,scale_v,scale_T2,scale_ener,scale_mag,microG,km_s,Cwnm,scale_mass,unit_col,lbox_pc = normalisation(ro)
        ...
        np.savez(name_save_pdf,hist_logrho=hist_logrho,pdf_logrho=pdf_logrho,histrho_edges=histrho_edges,histrhomv_edges=histrhomv_edges,pdf_T=pdf_T,width=width,pdf_B=pdf_B)
...
#    P.yscale('log', nonposy='clip')
    ...
    name_im = directory_out+'pdfmw_logrho_'+tag+str(num).zfill(5)
    if(ps):
    ...
    dd = dict()
    dd.update({'name':'mass weighted density PDF'})
    dd.update({'type':'histogram'})
    dd.update({'description':'Mass weighted density PDF.','units-x':'cm^{-3}','units-y':'Msun','xtitle':xlabel,'ytitle':ylabel})
    data={'png':name_im,'x-axis':histrho_edges,'y-axis':hist_logrho}
    dd.update({'data':data})
    list_plot.append(dd)
    ...
    name_im = directory_out+'pdfvw_logrho_'+tag+str(num).zfill(5)
    if(ps):
        P.savefig(name_im+'.ps')
    P.savefig(name_im+'.png')
    ...
    dd = dict()
    dd.update({'name':'volume weighted density PDF'})
    dd.update({'type':'histogram'})
    dd.update({'description':'Volume weighted density PDF.','units-x':'cm^{-3}','units-y':'none','xtitle':xlabel,'ytitle':ylabel})
    data={'png':name_im,'x-axis':histrho_edges,'y-axis':pdf_logrho}
    dd.update({'data':data})
    list_plot.append(dd)
    ...
    #mass weighted histogram of temperature 
    P.clf()
...
#    P.yscale('log', nonposy='clip')
    ...
    name_im = directory_out+'pdf_T_'+tag+str(num).zfill(5)
    if(ps):
        P.savefig(name_im+'.ps')
    P.savefig(name_im+'.png')
    ...
    dd = dict()
    dd.update({'name':'mass weighted temperature'})
    dd.update({'type':'histogram'})
    dd.update({'description':'Mass weighted temperature.','units-x':'cm^{-3}','units-y':'K','xtitle':xlabel,'ytitle':ylabel})
    data={'png':name_im,'x-axis':histrho_edges,'y-axis':pdf_T}
    dd.update({'data':data})
    list_plot.append(dd)
    ...
    if(mag_im):
        ...
        name_im = directory_out+'pdf_B_'+tag+str(num).zfill(5)
        if(ps):
            P.savefig(name_im+'.ps')
        P.savefig(name_im+'.png')
        ...
        dd = dict()
        dd.update({'name':'volume weighted magnetic intensity'})
        dd.update({'type':'histogram'})
        dd.update({'description':'Volume weighted magnetic intensity.','units-x':'cm^{-3}','units-y':'uG','xtitle':xlabel,'ytitle':ylabel})
        data={'png':name_im,'x-axis':histrho_edges,'y-axis':pdf_B}
        dd.update({'data':data})
        list_plot.append(dd)
    ...
    #store the list of dictionnaries
    name_im=directory_out+'/list_pdf'+'_'+tag+format(num,'05')+'.save'
    f = open(name_im,'w')
...
########################################################################################
########################################################################################
def make_image(amr,ro,center,radius,num,path,x_v=None,y_v=None,z_v=None,mass_v=None,i_im=0,force=False,path_out=None,col_dens_only=False,map_size=512,vel_red=20,im_pos=0.,tag='',ps=False,all_sink=True,savetxt=False,descrip=None):
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/coldens_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        name_txt=directory+'/coldens_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        map_col2 = np.log10(datamap.map*lbox_cm)
        np.savetxt(name_txt,map_col2,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/coldens_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_col=map_col2)
        ...
        map_Vz = datamap_vz.map / datamap.map * scale_v / km_s

        name_txt=directory+'/Vz_z_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Vz,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/Vz_z_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_Vz=map_Vz)


        Vz2_op = ScalarOperator(lambda dset: (dset["vel"][...,2])**2*dset["rho"] ,  ro.info["unit_velocity"])
        rt = raytracing.RayTracer(amr,ro.info,Vz2_op)
        datamap_vz2 = rt.process(cam_z, surf_qty=True)
        map_Vz2 = datamap_vz2.map / datamap.map * (scale_v / km_s)**2

        map_sigz = np.sqrt(map_Vz2 - map_Vz**2)

        name_txt=directory+'/sig_z_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_sigz,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/sig_z_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_sigz=map_sigz)


    if(descrip is not None):
        dd = dict()
        dd.update({'name':'colonne density along z'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density along the z-direction'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#        P.scatter((x_v+center[0])*lbox,(y_v+center[1])*lbox,c='r',marker='x',s=10)
        ...
        name=directory+'/coldens_z'+'_'+tag+str(i_im)+'_nscat_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            ...
            dd.update({'name':'colonne density along z'})
            dd.update({'type':'image'})
            dd.update({'description':'Column density along the z-direction. The dots represent the sink particles.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title(titre)
        ...
        if(savetxt):
            map_vx = dmap_vx.map*scale_v / km_s
            name_txt=directory+'/Vx_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vx,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            map_vy = dmap_vy.map*scale_v / km_s
            name_txt=directory+'/Vy_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vy,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            name_npz=directory+'/V_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
            length=2.*radius*lbox
            np.savez(name_npz,map_size=map_size,length=length,map_vx=map_vx,map_vy=map_vy)


        if(descrip is not None):
            dd = dict()
            dd.update({'name':'density cut in the xy-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Density in the xy-plane. The arrows represent the velocity field in the xy plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title=(titre,horizontalalignment='right')
        ...
        if(descrip is not None):
            dd = dict()
            dd.update({'name':'Temperature cut in the xy-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Temperature in the xy-plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/level_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            dd = dict()
            dd.update({'name':'mean AMR level in the z-direction'})
            dd.update({'type':'image'})
...
#        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/cpu_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            dd = dict()
            dd.update({'name':'Mean CPU number in the z-direction'})
            dd.update({'type':'image'})
            dd.update({'description':'Mean CPU number in the z-direction.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/coldens_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'colonne density along y'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density along the y-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#        P.scatter(x_v,z_v,c='r',marker='x',s=10)
        ...
        name=directory+'/coldens_y'+'_'+tag+str(i_im)+'_nscat_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            ...
            dd.update({'name':'colonne density along y'})
            dd.update({'type':'image'})
            dd.update({'description':'Column density along the y-direction. The dots represent the sink particles.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/rho_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(savetxt):
            map_vx = dmap_vx.map*scale_v / km_s
            name_txt=directory+'/Vx_y'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vx,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            map_vz = dmap_vz.map*scale_v / km_s
            name_txt=directory+'/Vz_y'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vz,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            name_npz=directory+'/V_y'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
            length=2.*radius*lbox
            np.savez(name_npz,map_size=map_size,length=length,map_vx=map_vx,map_vz=map_vz)



        if(descrip is not None):
            dd = dict()
            dd.update({'name':'density cut in the xz-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Density in the xz-plane. The arrows represent the velocity field in the xz-plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
##        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/T_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            dd = dict()
            dd.update({'name':'temperature cut in the xz-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Temperature in the xz-plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/coldens_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        map_col2 = np.log10(datamap.map*lbox_cm)
        name_txt=directory+'/coldens_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_col2,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/coldens_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_col=map_col2)
        ...
        map_Vx = datamap_vx.map.T / datamap.map.T * scale_v / km_s

        name_txt=directory+'/Vx_x_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Vx,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/Vx_x_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_Vx=map_Vx)



        Vx2_op = ScalarOperator(lambda dset: (dset["vel"][...,0])**2*dset["rho"] ,  ro.info["unit_velocity"])
        rt = raytracing.RayTracer(amr,ro.info,Vx2_op)
        datamap_vx2 = rt.process(cam_x, surf_qty=True)
        map_Vx2 = datamap_vx2.map / datamap.map * (scale_v / km_s)**2

        map_sigx = np.sqrt(map_Vx2 - map_Vx**2)

        name_txt=directory+'/sig_x_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_sigx,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/sig_x_int'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_sigx=map_sigx)


    if(descrip is not None):
        dd = dict()
        dd.update({'name':'colonne density along x'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density along the x-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#        P.scatter(y_v,z_v,c='r',marker='x',s=10)
        ...
        name=directory+'/coldens_x'+'_'+tag+str(i_im)+'_nscat_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            ...
            dd.update({'name':'colonne density along x'})
            dd.update({'type':'image'})
            dd.update({'description':'Column density along the x-direction. The dots represent the sink particles.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/rho_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(savetxt):
            map_vy = dmap_vy.map*scale_v / km_s
            name_txt=directory+'/Vy_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vy,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            map_vz = dmap_vz.map*scale_v / km_s
            name_txt=directory+'/Vz_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
            head = str(map_size)+ ' '+ str(2.*radius*lbox)
            np.savetxt(name_txt,map_vz,fmt='%.5e',header=head,delimiter=' ',comments=' ')

            name_npz=directory+'/V_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
            length=2.*radius*lbox
            np.savez(name_npz,map_size=map_size,length=length,map_vy=map_vy,map_vz=map_vz)

        if(descrip is not None):
            dd = dict()
            dd.update({'name':'density cut in the yz-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Density in the yz-plane. The arrows represent the velocity field in the yz-plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
...
#        P.title=(titre,horizontalalignment='right')
        ...
        name=directory+'/T_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
        name_im=name+'.jpeg'
        P.savefig(name_im)
        if(ps):
            P.savefig(name+'.ps')
        ...
        if(descrip is not None):
            dd = dict()
            dd.update({'name':'temperature cut in the yz-plane'})
            dd.update({'type':'image'})
            dd.update({'description':'Temperature in the yz-plane.'})
            data={'jpeg':name_im}
            dd.update({'data':data})
            list_im.append(dd)
    ...
    if(descrip is not None):
        name_im=directory+'/list_im'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.save'
        f = open(name_im,'w')
        pickle.dump(list_im,f)
        f.close()
...
########################################################################################
########################################################################################
def make_image_B(amr,ro,center,radius,num,path,x_v=None,y_v=None,z_v=None,mass_v=None,i_im=0,force=False,path_out=None,col_dens_only=False,map_size=512,vel_red=20,im_pos=0.,tag='',ps=False,savetxt=False,descrip=None):
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        name_txt=directory+'/Bx_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Bx.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_txt=directory+'/By_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_By.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/B_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_bx=map_Bx.T,map_by=map_By.T)


    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated B along z'})
        dd.update({'type':'image'})
        dd.update({'description':'Density weighted integrated magnetic intensity along the z-direction. The arrows represent the projected direction of the integrated B in the xy-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_z_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of B in the xy-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of the magnetic intensity in the xy-plane. The arrows represent the projected direction of B in the xy-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated B along y'})
        dd.update({'type':'image'})
        dd.update({'description':'Density weighted integrated magnetic intensity along the y-direction. The arrows represent the projected direction of the integrated B in the xz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_y_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of B in the xz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of the magnetic intensity in the xz-plane. The arrows represent the projected direction of B in the xz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        name_txt=directory+'/By_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_By.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_txt=directory+'/Bz_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Bz.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/B_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_by=map_By.T,map_bz=map_Bz.T)
...
#        pdb.set_trace() # debug mode     
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated B along x'})
        dd.update({'type':'image'})
        dd.update({'description':'Density weighted integrated magnetic intensity along the y-direction. The arrows represent the projected direction of B in the yz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/B_x_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of B in the yz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of the magnetic intensity in the yz-plane. The arrows represent the projected direction of B in the yz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
    ...
    #save the list of images
    if(descrip is not None):
        name_im=directory+'/list_im_mag'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.save'
        f = open(name_im,'w')
        pickle.dump(list_im,f)
        f.close()
...
########################################################################################
########################################################################################
##make images of yje absolute value of the  magnetic field
##amr and ro pymses stuff
##center 3D array for coordinates center
##radius the size of the region to be plotted (between 0 and 1)
##num output number
...
########################################################################################
########################################################################################
def make_image_absB(amr,ro,center,radius,num,path,x_v=None,y_v=None,z_v=None,mass_v=None,i_im=0,force=False,path_out=None,col_dens_only=False,map_size=512,vel_red=20,im_pos=0.,tag='',ps=False,savetxt=False,descrip=None):
...
#    titre = 't='+ str(time)[0:5] +' (Myr)'
    ...
    Bx_op = ScalarOperator(lambda dset: abs(dset["Br"][...,0])*dset["Br"][...,1]/abs(dset["Br"][...,1])*dset["rho"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,Bx_op)
    datamap = rt.process(cam_z, surf_qty=True)
    map_Bx = datamap.map.T / map_rho


    By_op = ScalarOperator(lambda dset: abs(dset["Br"][...,1])*dset["rho"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,By_op)
    datamap = rt.process(cam_z, surf_qty=True)
    map_By = datamap.map.T / map_rho
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/absB_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        name_txt=directory+'/absBx_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Bx.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_txt=directory+'/absBy_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_By.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/absB_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_bx=map_Bx.T,map_by=map_By.T)
...
#    map_rho = datamap.T
    ...
    Bx_op = ScalarOperator(lambda dset: abs(dset["Br"][...,0])*dset["rho"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,Bx_op)
    datamap = rt.process(cam_y, surf_qty=True)
#    map_Bx = datamap.T / map_rho
    map_Bx = datamap.map / map_rho
...
#    map_By = datamap.T / map_rho
    ...
    Bz_op = ScalarOperator(lambda dset: abs(dset["Br"][...,2])*(dset["Br"][...,0])/abs(dset["Br"][...,0])*dset["rho"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,Bz_op)
    datamap = rt.process(cam_y, surf_qty=True)
    map_Bz = datamap.map / map_rho
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/absB_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    Bz_op = ScalarOperator(lambda dset: abs(dset["Br"][...,2])*(dset["Br"][...,1])/abs(dset["Br"][...,1])*dset["rho"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,Bz_op)
    datamap = rt.process(cam_x, surf_qty=True)
    map_Bz = datamap.map.T / map_rho
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/absB_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(savetxt):
        name_txt=directory+'/absBy_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_By.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_txt=directory+'/absBz_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.txt'
        head = str(map_size)+ ' '+ str(2.*radius*lbox)
        np.savetxt(name_txt,map_Bz.T,fmt='%.5e',header=head,delimiter=' ',comments=' ')

        name_npz=directory+'/absB_x'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.npz'
        length=2.*radius*lbox
        np.savez(name_npz,map_size=map_size,length=length,map_by=map_By.T,map_bz=map_Bz.T)
...
########################################################################################
########################################################################################
def make_image_ext(amr,ro,center,radius,num,path,x_v=None,i_im=0,force=False,path_out=None,map_size=512,im_pos=0.,tag='',ps=False,savetxt=False,descrip=None):
...
#    titre = 't='+ str(time)[0:5] +' (Myr)'
    ...
    name = directory+'/ext_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.jpeg'
    if (len(glob.glob(name))==1 and not force):
        return 
    ...
    #do an integration 
    ext_op = ScalarOperator(lambda dset: dset["ext"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,ext_op)
    datamap = rt.process(cam_z, surf_qty=True)
    map_ext = np.log10(datamap.map.T)


    P.clf() 
    im = P.imshow(map_ext,extent=[(-radius+center[0])*lbox,(radius+center[0])*lbox,(-radius+center[1])*lbox,(radius+center[1])*lbox],origin='lower')   
...
#    P.title=(titre,horizontalalignment='right')
    ...
    P.ylabel('$y$ (pc)')
    cbar = P.colorbar(im)                                                                              
    cbar.set_label(r'$log(extinction) $')           

    name=directory+'/ext_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated extinction along z'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated extinction along the z-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#now do a cut
    ext_op = ScalarOperator(lambda dset: dset["ext"] ,  ro.info["unit_velocity"])
    dmap_ext = slicing.SliceMap(amr, cam_z, ext_op, z=0.) #,verbose=False)
    map_ext = np.log10(dmap_ext.map.T)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(extinction)$')                                                                                                   
    name=directory+'/ext_z_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of extinction in the xy-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of extinction in the xy-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/ext_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated extinction along y'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated extinction along the y-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#now do a cut
    ext_op = ScalarOperator(lambda dset: dset["ext"] ,  ro.info["unit_velocity"])
    dmap_ext = slicing.SliceMap(amr, cam_y, ext_op, z=0.) #,verbose=False)
    map_ext = np.log10(dmap_ext.map.T)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(extinction)$')                                                                                                   
    name=directory+'/ext_y_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of extinction in the xz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of extinction in the xz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/ext_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated extinction along x'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated extinction along the x-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#now do a cut
    ext_op = ScalarOperator(lambda dset: dset["ext"] ,  ro.info["unit_velocity"])
    dmap_ext = slicing.SliceMap(amr, cam_x, ext_op, z=0.) #,verbose=False)
    map_ext = np.log10(dmap_ext.map.T)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(extinction)$')                                                                                                   
    name=directory+'/ext_x_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of extinction in the yz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of extinction in the yz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
    ...
    #save the list of images
    if(descrip is not None):
        name_im=directory+'/list_im_ext'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.save'
        f = open(name_im,'w')
        pickle.dump(list_im,f)
        f.close()
...
########################################################################################
########################################################################################
##make images of the self-shielding from H2
##amr and ro pymses stuff
##center 3D array for coordinates center
##radius the size of the region to be plotted (between 0 and 1)
##num output number
...
########################################################################################
########################################################################################
def make_image_selfshield(amr,ro,center,radius,num,path,x_v=None,i_im=0,force=False,path_out=None,map_size=512,im_pos=0.,tag='',ps=False,savetxt=False,descrip=None):
...
#    titre = 't='+ str(time)[0:5] +' (Myr)'
    ...
    name = directory+'/ssh_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.jpeg'
    if (len(glob.glob(name))==1 and not force):
        return 
    ...
    #do an integration 
    ext_op = ScalarOperator(lambda dset: dset["ssh"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,ext_op)
    datamap = rt.process(cam_z, surf_qty=True)
    map_ext = np.log10(datamap.map.T)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    P.ylabel('$y$ (pc)')
    cbar = P.colorbar(im)                                                                              
    cbar.set_label(r'$log(self-schielding) $')           

    name=directory+'/ssh_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated self-shielding of H2 along z'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated self-shielding of H2 along the z-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    P.ylabel('$y$ (pc)')
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(self-shielding)$') 
    name=directory+'/ssh_z_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of self-shielding in the xy-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of self-shielding in the xy-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/ssh_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated self-shielding of H2 along y'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated self-shielding of H2 along the y-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(self-shielding)$') 
    name=directory+'/ssh_y_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of self-shielding in the xz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of self-shielding in the xz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/ssh_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'integrated self-shielding of H2 along x'})
        dd.update({'type':'image'})
        dd.update({'description':' integrated self-shielding of H2 along the x-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(self-shielding)$')
    name=directory+'/ssh_x_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of self-shielding of H2 in the yz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of self-shielding of H2 in the yz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
    ...
    #save the list of images
    if(descrip is not None):
        name_im=directory+'/list_im_ssh'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.save'
        f = open(name_im,'w')
        pickle.dump(list_im,f)
        f.close()
...
########################################################################################
########################################################################################
def make_image_h2(amr,ro,center,radius,num,path,x_v=None,i_im=0,force=False,path_out=None,map_size=512,im_pos=0.,tag='',ps=False,savetxt=False,descrip=None):
...
#    titre = 't='+ str(time)[0:5] +' (Myr)'
    ...
    name = directory+'/H2_z'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.jpeg'
    if (len(glob.glob(name))==1 and not force):
        return 
    ...
    #do an integration 
    ext_op = ScalarOperator(lambda dset: dset["h2"] ,  ro.info["unit_velocity"])
    rt = raytracing.RayTracer(amr,ro.info,ext_op)
    datamap = rt.process(cam_z, surf_qty=True)
    map_ext = np.log10(datamap.map.T*lbox_cm)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    P.ylabel('$y$ (pc)')
    cbar = P.colorbar(im)                                                                              
    cbar.set_label(r'$log(N_{H2}) $')           

    name=directory+'/h2_z'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'column density of H2 along z'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density of H2 along the z-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(n_{H2})$') 
    name=directory+'/h2_z_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of H2 density in the xy-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of H2 density in the xy-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/h2_y'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'Column density of H2 along y'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density of H2 along the y-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of H2 density in the xz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of H2 density in the xz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    name=directory+'/h2_x'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'Column density of H2 along x'})
        dd.update({'type':'image'})
        dd.update({'description':'Column density of H2 along the x-direction.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
...
#    P.title=(titre,horizontalalignment='right')
    ...
    cbar = P.colorbar(im)                                                                                                                     
    cbar.set_label(r'$log(n_{H2})$')
    name=directory+'/h2_x_cut'+'_'+tag+str(i_im)+'_'+format(num,'05')
    name_im=name+'.jpeg'
    P.savefig(name_im)
    if(ps):
        P.savefig(name+'.ps')
    ...
    if(descrip is not None):
        dd = dict()
        dd.update({'name':'cut of H2 density in the yz-plane'})
        dd.update({'type':'image'})
        dd.update({'description':'Cut of H2 density in the yz-plane.'})
        data={'jpeg':name_im}
        dd.update({'data':data})
        list_im.append(dd)
    ...
    #save the list of images
    if(descrip is not None):
        name_im=directory+'/list_im_h2'+'_'+tag+str(i_im)+'_'+format(num,'05')+'.save'
        f = open(name_im,'w')
        pickle.dump(list_im,f)
        f.close()
...
##############################################################################
##############################################################################
### get normalisation for code quantities (modified by Verliat Antoine)
##############################################################################
##############################################################################
def normalisation_averliat(ro):


    lbox=ro.info['boxlen'] #boxlen in codeunits (=>pc)
    mu=ro.info['mu_gas'] #boxlen in codeunits (=>pc)

    AU = 1.495978e13 #au in cm  #ANTOINE VERLIAT ajout de cette definition car sinon extract_disk ne marche pas
...
#lbox en pc
    ...
    return  AU,pc,Ms,Myr,scale_n,scale_d,scale_t,scale_l,scale_v,scale_T2,scale_ener,scale_mag,microG,km_s,Cwnm,scale_mass,unit_col,lbox_pc
##############################################################################
##############################################################################
### get normalisation for code quantities
##############################################################################
##############################################################################
def normalisation(ro):
...
##############################################################################
##############################################################################
def make_image_zoom(path,num,zoom_v,path_out=None,sinks=False,force=True,center_zoom=None,make_phi=False,no_cvs=None,col_dens_only=False,tag='',gcomp=True,mag_im=True,vel_red=20,map_size=512,im_pos=0.,conv_sink=1,center_dmax=False,order='<',ps=False,ind_sink=0,all_sink=True,savetxt=False,descrip=None,extinct=False,extinct2=False,mag_abs=False):
...
#        amr = ro.amr_source(["rho","vel","P"])
    ...
    if(extinct):
        amr_ext = ro.amr_source(["ext"])


    if(extinct2):
        amr_ext2 = ro.amr_source(["ssh","h2"])
...
#            ind_max = np.argmax(sinks[:,1])
    ...
    for rad in zoom_v:
        ...
        if(not make_phi):
            make_image(amr,ro,center,rad,num,path,x_v=x_v,y_v=y_v,z_v=z_v,mass_v=mass_v,i_im=i_im,force=force,path_out=path_out,col_dens_only=col_dens_only,tag=tag,vel_red=vel_red,map_size=map_size,im_pos=im_pos,ps=ps,all_sink=all_sink,savetxt=savetxt)
            if (not col_dens_only and mag_im):
                make_image_B(amr,ro,center,rad,num,path,x_v=None,y_v=None,z_v=None,mass_v=None,i_im=i_im,force=force,path_out=path_out,tag=tag,ps=ps,savetxt=savetxt)


            if (not col_dens_only and mag_abs):
                make_image_absB(amr,ro,center,rad,num,path,x_v=None,y_v=None,z_v=None,mass_v=None,i_im=i_im,force=force,path_out=path_out,tag=tag,ps=ps,savetxt=savetxt)
        ...
        if(extinct):
            make_image_ext(amr_ext,ro,center,rad,num,path,i_im=i_im,force=force,path_out=path_out,tag=tag,ps=ps)


        if(extinct2):
            make_image_selfshield(amr_ext2,ro,center,rad,num,path,i_im=i_im,force=force,path_out=path_out,tag=tag,ps=ps)

            make_image_h2(amr_ext2,ro,center,rad,num,path,i_im=i_im,force=force,path_out=path_out,tag=tag,ps=ps)
...
##############################################################################
##############################################################################
### do images of the last output in a path
### path : the path
### zoom_v an arrays which contains the zoom
### sinks particle can be overplotted and used to place the zoom
##############################################################################
##############################################################################
def make_image_zoom_last(path,zoom_v,path_out=None,sinks=False,force=True,center_zoom=None,make_phi=False,no_cvs=None,col_dens_only=False,tag='',gcomp=True,mag_im=True,vel_red=20,map_size=512,im_pos=0.,conv_sink=1,center_dmax=False,order='<',ps=False,ind_sink=0,all_sink=True,savetxt=False):


    def getkey2(item):
        ntot=len(item)
        key=int(item[ntot-5:ntot])
        return key


    last_out =  sorted(glob.glob(path+"output_*"),key=getkey2,reverse=True)[0]

    ntot=len(last_out)
    num=int(last_out[ntot-5:ntot])

    print last_out,' is the last output. Treating it.'
    print 'num ', num

    make_image_zoom(path,num,zoom_v,path_out,sinks=sinks,force=force,center_dmax=center_dmax,ps=ps)

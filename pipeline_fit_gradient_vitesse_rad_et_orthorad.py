...
dir_save = 'Gradient_vitesse_rad_et_orthorad'  #Repertoire de sauvegarde des figures
...
file_save_ang = 'Angle_omega_J_gradient_vitesse_rad_et_orthorad_log_output'+str(num_output)+'_Rmin'+str(R_min)+'_Rmax'+str(R_max)+'_dr'+str(dr)+'.hdf5'
...
#-----------------------------------------
#Definition de l'operateur vitesse radiale
#-----------------------------------------
def vel_rad_op(dset):
    rho = dset["rho"]
    vel = dset["vel"]
    pos = dset.get_cell_centers()
    pos_rel_vect = pos - center #vecteur centre --> point
    pos_rel_norm = np.sqrt( np.sum( (pos_rel_vect)**2 , axis=2 ) )  #Tableau contenant le rayon de chaque cellule
    pos_rel_unit = pos_rel_vect / pos_rel_norm[:,:,None]
    #Pour voir les nan : np.argwhere(np.isnan(x))
    pos_rel_unit = np.nan_to_num(pos_rel_unit)
    vel_rad_norm = np.sum(vel*pos_rel_unit,axis=2)
    vel_rad_vect = vel_rad_norm[:,:,None] * pos_rel_unit
    vel_rad_selon_los = np.sum(vel_rad_vect*los,axis=2) / np.sum(los**2)
    vel_rad_selon_los *= dset["rho"]
    return vel_rad_selon_los
#-------------------------------------------
#-------------------------------------------


#---------------------------------------------
#Definition de l'operateur vitesse orthradiale
#---------------------------------------------
def vel_orthorad_op(dset):
    rho = dset["rho"]
    vel = dset["vel"]
    pos = dset.get_cell_centers()
    pos_rel_vect = pos - center #vecteur centre --> point
    ...
    vel_rad_norm = np.sum(vel*pos_rel_unit,axis=2)
    vel_rad_vect = vel_rad_norm[:,:,None] * pos_rel_unit
    vel_orthorad_vect = vel - vel_rad_vect
    vel_orthorad_selon_los = np.sum(vel_orthorad_vect*los,axis=2) / np.sum(los**2)
    vel_orthorad_selon_los *= dset["rho"]
    return vel_orthorad_selon_los
#-------------------------------------------
#-------------------------------------------
...
ang_x_rad_tab=np.zeros(nbr_shell)
ang_y_rad_tab=np.zeros(nbr_shell)
ang_z_rad_tab=np.zeros(nbr_shell)
omega_x_rad_tab=np.zeros(nbr_shell)
omega_y_rad_tab=np.zeros(nbr_shell)
omega_z_rad_tab=np.zeros(nbr_shell)
moment_spec_x_rad_tab=np.zeros(nbr_shell)
moment_spec_y_rad_tab=np.zeros(nbr_shell)
moment_spec_z_rad_tab=np.zeros(nbr_shell)
ang_x_orthorad_tab=np.zeros(nbr_shell)
ang_y_orthorad_tab=np.zeros(nbr_shell)
ang_z_orthorad_tab=np.zeros(nbr_shell)
omega_x_orthorad_tab=np.zeros(nbr_shell)
omega_y_orthorad_tab=np.zeros(nbr_shell)
omega_z_orthorad_tab=np.zeros(nbr_shell)
moment_spec_x_orthorad_tab=np.zeros(nbr_shell)
moment_spec_y_orthorad_tab=np.zeros(nbr_shell)
moment_spec_z_orthorad_tab=np.zeros(nbr_shell)
ind=0
verif=0
...
for radius in shells:
    if selon_x==True:
        ...
        Vx_op = ScalarOperator(vel_rad_op, ro.info["unit_velocity"])
        ...
        (ang_x_rad_tab[ind],omega_x_rad_tab[ind],moment_spec_x_rad_tab[ind])=fit_gradient_vel((map_Vx,extent), 'x_rad',radius=radius)
        ...
        #====================
        #Vx_op = ScalarOperator(lambda dset: dset["vel"][...,0]*dset["rho"] ,  ro.info["unit_velocity"])
        Vx_op = ScalarOperator(vel_orthorad_op, ro.info["unit_velocity"])
        rt = raytracing.RayTracer(amr,ro.info,Vx_op)
        datamap_vx = rt.process(cam_x, surf_qty=True)
        map_Vx = datamap_vx.map.T / datamap.map.T * factor_vel_km_s
        ...
        #Fit du gradient de vitesse
        (ang_x_orthorad_tab[ind],omega_x_orthorad_tab[ind],moment_spec_x_orthorad_tab[ind])=fit_gradient_vel((map_Vx,extent), 'x_orthorad',radius=radius)
    ...
    if selon_y==True:
        ...
        Vy_op = ScalarOperator(vel_rad_op, ro.info["unit_velocity"])
        ...
        #Fit du gradient de vitesse
        (ang_y_rad_tab[ind],omega_y_rad_tab[ind],moment_spec_y_rad_tab[ind])=fit_gradient_vel((map_Vy,extent), 'y_rad',radius=radius)
        ...
        #Vitesse orthoradiale
        #====================
        Vy_op = ScalarOperator(vel_orthorad_op, ro.info["unit_velocity"])
        rt = raytracing.RayTracer(amr,ro.info,Vy_op)
        datamap_vy = rt.process(cam_y, surf_qty=True)
        map_Vy = datamap_vy.map.T / datamap.map.T * factor_vel_km_s
        ...
        #Fit du gradient de vitesse
        (ang_y_orthorad_tab[ind],omega_y_orthorad_tab[ind],moment_spec_y_orthorad_tab[ind])=fit_gradient_vel((map_Vy,extent), 'y_orthorad',radius=radius)
    ...
    if selon_z==True:
        ...
        Vz_op = ScalarOperator(vel_rad_op, ro.info["unit_velocity"])
        ...
        #Fit du gradient de vitesse
        (ang_z_rad_tab[ind],omega_z_rad_tab[ind],moment_spec_z_rad_tab[ind])=fit_gradient_vel((map_Vz,extent), 'z_rad',radius=radius)
        ...
        #Vitesse orthoradiale
        #====================
        Vz_op = ScalarOperator(vel_orthorad_op, ro.info["unit_velocity"])
        rt = raytracing.RayTracer(amr,ro.info,Vz_op)
        datamap_vz = rt.process(cam_z, surf_qty=True)
        map_Vz = datamap_vz.map.T / datamap.map.T * factor_vel_km_s
        ...
        #Fit du gradient de vitesse
        (ang_z_orthorad_tab[ind],omega_z_orthorad_tab[ind],moment_spec_z_orthorad_tab[ind])=fit_gradient_vel((map_Vz,extent), 'z_orthorad',radius=radius)
...
if (save_ang == True and verif==1):
    ...
    h5f.create_dataset('ang_x_rad_tab',data=ang_x_rad_tab)
    h5f.create_dataset('ang_y_rad_tab',data=ang_y_rad_tab)
    h5f.create_dataset('ang_z_rad_tab',data=ang_z_rad_tab)
    h5f.create_dataset('omega_x_rad_tab',data=omega_x_rad_tab)
    h5f.create_dataset('omega_y_rad_tab',data=omega_y_rad_tab)
    h5f.create_dataset('omega_z_rad_tab',data=omega_z_rad_tab)
    h5f.create_dataset('moment_spec_x_rad_tab',data=moment_spec_x_rad_tab)
    h5f.create_dataset('moment_spec_y_rad_tab',data=moment_spec_y_rad_tab)
    h5f.create_dataset('moment_spec_z_rad_tab',data=moment_spec_z_rad_tab)
    h5f.create_dataset('ang_x_orthorad_tab',data=ang_x_orthorad_tab)
    h5f.create_dataset('ang_y_orthorad_tab',data=ang_y_orthorad_tab)
    h5f.create_dataset('ang_z_orthorad_tab',data=ang_z_orthorad_tab)
    h5f.create_dataset('omega_x_orthorad_tab',data=omega_x_orthorad_tab)
    h5f.create_dataset('omega_y_orthorad_tab',data=omega_y_orthorad_tab)
    h5f.create_dataset('omega_z_orthorad_tab',data=omega_z_orthorad_tab)
    h5f.create_dataset('moment_spec_x_orthorad_tab',data=moment_spec_x_orthorad_tab)
    h5f.create_dataset('moment_spec_y_orthorad_tab',data=moment_spec_y_orthorad_tab)
    h5f.create_dataset('moment_spec_z_orthorad_tab',data=moment_spec_z_orthorad_tab)

...
#------------------
    ...
    colorspts = [cmappts(i) for i in np.linspace(0.1,0.82,len(tag))]
    ...
    plt.figure(17)
    plt.semilogx(shells_au, ang_rad_tab-ang_tab,linestyle='solid',color=colorspts[ind], label=view+'_rad'+str(tag[ind]))
    plt.xlabel(ur'Rayon ($AU$)')
    plt.ylabel(ur"Angle (°)")
    plt.legend(loc='best')

    #plt.figure()
    plt.semilogx(shells_au, ang_orthorad_tab-ang_tab,linestyle='dashed',color=colorspts[ind], label=view+'_orthorad'+str(tag[ind]))
    plt.xlabel(ur'Rayon ($AU$)')
    plt.ylabel(ur"Angle (°)")
    plt.legend(loc='best')

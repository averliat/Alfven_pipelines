...
#------------------
    ...
    if indice==len(tagfull)-1:
        ...
        norot_legend=ax.legend([handle for i,handle in enumerate(handles) if i in display_norot],
                [label for i,label in enumerate(labels) if i in display_norot],loc='upper right',ncol=1,fancybox=True)#,framealpha=0.1)
        ax.add_artist(norot_legend)
        #ax.add_artist(norot_legend)
        #norot_legend.get_frame().set_alpha(0.1)
        #norot_legend.set_frame_on(True)
        #norot_legend.get_frame().set_edgecolor('black')
        ...
        #ax.add_artist(MHD_legend)
        #MHD_legend.get_frame().set_alpha(0.1)
        #MHD_legend.set_frame_on(True)
        #MHD_legend.get_frame().set_edgecolor('black')

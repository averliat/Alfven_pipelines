...
#tab_x = np.linspace(1,output_max,len(norm_mom_cin_obs_par_integ))
    ...
    if moyenne_glissante==True:
        ...
        norm_mom_cin_obs_par_integ2=norm_mom_cin_obs_par_integ2[0:frag]
        norm_mom_cin_obs_par_integ=norm_mom_cin_obs_par_integ[0:frag]
        norm_mom_cin_obs=norm_mom_cin_obs[0:frag]
        simulation_time=simulation_time[0:frag]
        ...
        plt.figure(23)
        ax=plt.gca()
        plt.plot(time_lisse*1e3,erreur_lisse, marker=marker, color=colorspts[int(tag)/10-1], label=ur'$\epsilon=$ '+tag+ur'\%')
        ax.yaxis.set_ticks_position('right')
        ax.yaxis.set_label_position("right")
        plt.xlabel(ur'Time (kyr)')
        plt.ylabel(ur"$\frac{\left| \sigma_{\text{disk}} \right| - \left| \Delta \sigma \right|}{\left| \sigma_{\text{disk}} \right|}$")
